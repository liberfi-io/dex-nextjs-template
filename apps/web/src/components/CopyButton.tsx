...
export function CopyButton({
  ...
  return (
    <button
      ...
      onClick={copy}
      className={cn(
        "cursor-pointer rounded p-1 text-text-muted transition-colors hover:bg-surface-strong/60 hover:text-white",
        className,
      )}
    >
...
export function CopyInline({
  ...
  return (
    ...
    >
      {children}
      <span className="shrink-0 text-text-muted transition-colors group-hover/copy:text-text-secondary">
        <CopyGlyph copied={copied} size={size} />
      </span>
    </span>
  );
