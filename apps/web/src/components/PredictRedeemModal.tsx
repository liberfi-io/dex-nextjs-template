...
function RedeemForm({
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div className="flex flex-col gap-2 rounded-xl bg-surface-strong/40 p-3">
        <div className="min-w-0">
          <div className="line-clamp-2 text-base font-medium text-white">
            {positionTitle}
          </div>
          {(positionSubtitle || sideLabel) && (
            <div className="mt-1 flex min-w-0 items-center gap-2 text-sm text-text-muted">
              {positionSubtitle && (
                <span className="min-w-0 truncate">{positionSubtitle}</span>
              )}
              {positionSubtitle && sideLabel && (
                <span className="shrink-0 text-text-disabled">&bull;</span>
              )}
              {sideLabel && (
        ...
        <InfoRow
          ...
          valueClassName={
            expectedPayout !== undefined && expectedPayout > 0 ? "text-positive" : "text-text-muted"
          }
        />
      </div>
      ...
      {expectedPayout === 0 && (
        <p className="text-sm text-text-muted">{t("predict.redeem.noWinnings")}</p>
      )}
      ...
      {status === "success" && (
        <p className="text-sm text-positive">{t("predict.redeem.success")}</p>
      )}
      {status === "delayed" && (
      ...
      <div className="flex flex-col gap-2">
        {status !== "success" && status !== "delayed" && (
          <Button
            className="h-12 w-full rounded-xl bg-positive text-sm font-semibold text-black hover:bg-positive/90"
        ...
        {onCancel && status !== "success" && status !== "delayed" && (
          <button
            type="button"
            onClick={onCancel}
            className="h-12 w-full cursor-pointer rounded-xl text-sm font-medium text-text-muted transition-colors hover:bg-surface-strong/60 hover:text-text-primary focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus"
          >
            {t("common.cancel")}
          </button>
        )}
