...
function EventPositionsPanel({
  ...
  return (
    <div className="mt-4 divide-y divide-border-subtle/30 overflow-y-auto rounded-xl border border-border-subtle/30 bg-surface-raised/20" style={{ maxHeight: LIST_HEIGHT }}>
      {positions.map((pos, i) => {
        const pnl = pos.pnl ?? 0;
        ...
        return (
          <div key={`${pos.source}-${pos.market?.slug ?? i}-${pos.side || "unknown"}`} className="group transition-[background-color] duration-150 hover:bg-surface-interactive/30">
            {/* Desktop row */}
            <div className="hidden items-center gap-4 px-5 py-4 lg:flex">
              <div className="flex min-w-0 flex-1 items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 text-xs text-text-muted">
                    ...
                    <span className={cn("rounded px-1.5 py-0.5 font-medium", isYes ? "bg-positive/10 text-positive" : "bg-negative/10 text-negative")}>
                      {sideLabel}
                    </span>
                    <span className="text-text-disabled">&bull;</span>
              ...
              <div className="min-w-[90px] shrink-0 text-right">
                <div className="mb-0.5 text-xs text-text-muted">{t("extend.portfolio.invested")}</div>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-text-muted">
                  ...
                  <span className="text-text-muted">
                    ...
                    <span className={currentPrice > avgPrice ? "text-positive" : currentPrice < avgPrice ? "text-negative" : ""}>{formatPrice(currentPrice)}</span>
                  </span>
                </div>
              </div>
      ...
      <div className="relative w-full" style={{ height: virtualizer.getTotalSize() }}>
        {virtualizer.getVirtualItems().map((vItem) => {
          ...
          return (
            ...
            >
              <div className={cn("group transition-[background-color] duration-150 hover:bg-surface-interactive/30", vItem.index < orders.length - 1 && "border-b border-border-subtle/30")}>
                {/* Desktop */}
                <div className="hidden items-center gap-4 px-5 py-4 lg:flex">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border-subtle/50 bg-surface-raised">
                    {imageUrl ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {SHOW_SOURCE_BADGE && (
                      <div className="flex items-center gap-1.5 text-xs text-text-muted">
                        {source === "kalshi" ? <KalshiIcon width={36} height={12} /> : <><PolymarketIcon width={14} height={14} /><span>{t("predict.platform.polymarket")}</span></>}
                      </div>
                    )}
                  </div>
                  <div className="min-w-[100px] shrink-0 text-center">
                    <span className={cn("inline-block rounded px-2 py-1 text-xs font-semibold", isBuy ? "bg-positive/10 text-positive" : "bg-negative/10 text-negative")}>
                  ...
                  <div className="min-w-[100px] shrink-0 text-right">
                    ...
                    <div className="font-mono text-sm font-medium text-white">{order.size_matched ?? "0"}<span className="text-text-muted">/{order.original_size ?? "—"}</span></div>
                  </div>
                  {canCancel && (
                    <button
                      type="button"
                ...
                <div className="flex items-center gap-3 p-4 lg:hidden">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-1.5 text-xs text-text-muted">
                      <span className={cn("rounded px-1.5 py-0.5 font-semibold", isBuy ? "bg-positive/10 text-positive" : "bg-negative/10 text-negative")}>
                        {order.side} {order.outcome ? <span className="capitalize">{order.outcome}</span> : null}
                      </span>
                    </div>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className="font-mono text-sm font-medium text-white">{order.price ? formatPrice(parseFloat(order.price)) : "—"}</div>
                    <div className="font-mono text-xs text-text-muted">{order.size_matched ?? "0"}<span className="text-text-disabled">/{order.original_size ?? "—"}</span></div>
                  </div>
                  {canCancel && (
                    <button
                      type="button"
        ...
        {virtualItems.map((vItem) => {
          ...
          return (
            <div
              ...
              style={{ transform: `translateY(${vItem.start}px)` }}
            >
              <div className={cn("group transition-[background-color] duration-150 hover:bg-surface-interactive/30", vItem.index < trades.length - 1 && "border-b border-border-subtle/30")}>
                {/* Desktop */}
                <div className="hidden items-center gap-4 px-5 py-4 lg:flex">
                  <div className="flex min-w-0 flex-1 items-center gap-3">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border-subtle/50 bg-surface-raised">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {eventTitle && marketQuestion && eventTitle !== marketQuestion && (
                        <span className="mb-0.5 line-clamp-1 text-xs text-text-muted">{marketQuestion}</span>
                      )}
                      {SHOW_SOURCE_BADGE && (
                        <div className="flex items-center gap-1.5 text-xs text-text-muted">
                          <span className="inline-flex items-center gap-1">
                            {source === "kalshi" ? <KalshiIcon width={36} height={12} /> : <><PolymarketIcon width={14} height={14} /><span className="text-text-muted">{t("predict.platform.polymarket")}</span></>}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="min-w-[120px] shrink-0 text-center">
                    <span className={cn("inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-sm font-semibold", isRedeem ? "bg-primary/10 text-primary" : isBuy ? "bg-positive/10 text-positive" : "bg-negative/10 text-negative")}>
                ...
                <div className="flex items-center gap-3 p-4 lg:hidden">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-1.5 text-xs text-text-muted">
                      <span className={cn("inline-flex items-center gap-1 rounded-md px-2 py-0.5 font-semibold", isRedeem ? "bg-primary/10 text-primary" : isBuy ? "bg-positive/10 text-positive" : "bg-negative/10 text-negative")}>
                        {isRedeem ? t("predict.profile.redeem") : trade.side}
                      {outcomeLabel !== "—" && <span className="capitalize"> {outcomeLabel}</span>}
                      </span>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-12">
      <EmptyIcon width={32} height={32} className="text-text-disabled" />
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-12">
      ...
      <button
        type="button"
        onClick={onSignIn}
        className="inline-flex items-center gap-1.5 rounded-[10px] border border-brand-primary/25 bg-brand-primary/10 px-4 py-2 text-sm font-semibold text-brand-primary transition-colors duration-200 hover:border-brand-primary/40 hover:bg-brand-primary/20 cursor-pointer"
      >
