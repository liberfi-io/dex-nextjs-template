...
function SportsShellBody({
  ...
  return (
    <main className="h-full min-h-0 overflow-hidden bg-surface-base text-text-primary">
      <div className="mx-auto flex h-full w-full max-w-[1440px] min-h-0 flex-col lg:grid lg:grid-cols-[240px_minmax(0,1fr)]">
        <aside className="no-scrollbar hidden min-h-0 overflow-y-auto border-r border-border-subtle px-4 py-5 lg:block">
          <SportsNavigation
            section={section}
        ...
        <section className="flex min-h-0 min-w-0 flex-1 flex-col">
          <div
            ...
            className={cn(
              "shrink-0 bg-surface-base px-3 pt-3 sm:px-6 lg:px-8 lg:pt-5",
              !isLiveView &&
          ...
          >
            <div className="flex items-center gap-2 pb-3 lg:hidden">
              ...
              >
                <Link
                  href={`/predict/${section}?view=live`}
                ...
                >
                  <span className="flex items-center gap-1.5">
                    <LiveNavigationIcon className="h-4 w-4 text-negative" />
                    {t("extend.sports.filters.live")}
                  </span>
                </Link>
                <Link
                  href={`/predict/${section}?view=proposals`}
                  onClick={handleSpecialViewNavigate}
              ...
              <button
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border-control bg-surface-base text-text-secondary"
                ...
                <h1 className="text-xl font-semibold text-text-primary">
                  {isLiveView ? (
                    <span className="inline-flex items-center gap-1.5">
                      <LiveNavigationIcon className="h-[18px] w-[18px] text-negative" />
                      {t("extend.sports.filters.live")}{" "}
...
function NavigationGroup({
  ...
  return (
    <section className="py-4 first:pt-0 last:pb-0">
      <h2 className="mb-2 px-2 text-xs font-semibold uppercase tracking-wide text-text-muted">
        {title}
      </h2>
      {children}
    </section>
        ...
        {(["today", "games", "props"] as const).map((tab) => (
          <button
            ...
            className={cn(
              ...
              active === tab
                ? "bg-surface-interactive/70 text-brand-primary"
                : "text-text-muted hover:bg-surface-interactive/40 hover:text-text-secondary",
            )}
          >
...
const SPORTS_MATCH_GROUP_HEADING_CLASS =
  "flex items-center gap-3 bg-surface-base py-2 pl-4 pr-[17px]";
      ...
      <div className="hidden shrink-0 gap-2 md:flex">
        {categories.map((category) => (
          <span
            ...
            className={cn(
              "truncate text-[11px] font-semibold uppercase tracking-wide text-text-muted",
              categories.length === 1
                ? "w-[400px] text-right"
                : "w-[128px] text-center",
            )}
              ...
              <div
                ...
                className={cn(
                  ...
                  isActiveGroupHeading
                    ? "sticky top-0 z-20 bg-surface-base"
                    : "absolute top-0",
                )}
                style={
                  isActiveGroupHeading
              ...
              >
                {isActiveGroupHeading && (
                  <div
                    ...
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-x-0 -top-4 h-4 bg-surface-base"
                  />
                )}
                {row.kind === "heading" ? (
...
export function SportsMatchCard({
  ...
  const href = `/predict/event/${encodeURIComponent(match.match_group_slug)}`;
      ...
      <div className="flex items-center justify-between gap-2 px-3 pt-2.5 sm:px-4">
          ...
          {showLeague && match.league_slug ? (
            <>
              <span
                ...
                className="min-w-0 max-w-32 flex-1 truncate text-xs font-medium text-text-muted sm:max-w-48"
              >
                {leagueNode ? (
                  <LocalizedTaxonomyLabel
                    node={leagueNode}
              ...
              <span
                aria-hidden="true"
                className="shrink-0 text-xs text-text-disabled"
              >
                ·
              </span>
            </>
        ...
        {match.market_count ? (
          <button
            type="button"
            onClick={open}
            className="flex shrink-0 cursor-pointer items-center gap-1 rounded-full border border-border-control/60 bg-surface-interactive/50 px-2.5 py-1 text-[11px] font-medium text-text-secondary transition-colors hover:border-border-control/70 hover:bg-surface-interactive hover:text-text-primary"
          >
...
function SportsParticipantAvatar({
  ...
  return participant.logo_url ? (
    ...
    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-surface-interactive text-[11px] font-semibold text-text-secondary">
