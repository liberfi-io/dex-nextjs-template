...
export function SportsPropsList({
  ...
  }, [getScrollElement, page.items.length]);
      ...
      <div className="flex h-8 items-center justify-center">
        {loading && (
          <span
            ...
            className="h-4 w-4 animate-spin rounded-full border-2 border-border-control border-t-zinc-300"
            role="status"
          />
        )}
      </div>
    ...
    >
      ...
      <CardFooter>
        {activeMarket && !singleMarket ? (
          <button
            type="button"
            className="flex cursor-pointer items-center gap-1 text-[10px] text-text-muted transition-colors hover:text-text-secondary lg:text-xs"
          ...
          <Link
            href={`/predict/event/${encodeURIComponent(event.event_slug)}`}
            prefetch
            className="flex cursor-pointer items-center gap-1 text-[10px] text-text-muted underline-offset-2 transition-colors hover:text-text-secondary hover:underline focus-visible:text-text-secondary focus-visible:underline lg:text-xs"
          >
            {singleMarket
        ...
        <span className="text-[10px] tabular-nums text-text-muted lg:text-xs">
...
function CardHeader({ event }: { event: SportsPropEventCard }) {
  return (
    ...
    >
      ...
      <h2 className="min-w-0 flex-1 text-sm font-semibold leading-snug text-text-secondary transition-colors line-clamp-2 group-hover/detail-link:text-white group-focus-visible/detail-link:text-white sm:text-base">
        {event.title}
      </h2>
    </Link>
  );
  ...
  return (
    <div className="flex w-full flex-1 flex-col gap-y-0.5 lg:gap-y-2">
      {markets.map((market) => (
        ...
        >
          <span className="min-w-0 flex-1 text-left text-xs text-text-secondary line-clamp-1 lg:text-sm">
            {market.label}
          </span>
          <div className="flex shrink-0 items-center gap-2">
            <SportsPropOdds
              ...
              format={format}
              className="text-sm font-semibold text-text-secondary lg:text-lg"
            />
            <YesNoPill />
          </div>
        </button>
    ...
    <div className="flex flex-1 flex-col">
      {collapsible && (
        <button
          ...
          onClick={onCollapse}
        >
          <span className="min-w-0 flex-1 text-left text-xs text-text-secondary lg:text-sm">
            {market.label}
          </span>
...
function CardFooter({ children }: { children: ReactNode }) {
  return (
    <footer className="mt-auto flex min-h-9 items-center justify-between gap-3 border-t border-border-subtle/50 bg-surface-interactive/15 px-3.5 py-2">
      {children}
    </footer>
  );
}
...
function SportsPropImage({ event }: { event: SportsPropEventCard }) {
  ...
  if (!event.image_url || failed) {
    return (
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-surface-interactive text-xs font-semibold text-text-muted sm:h-12 sm:w-12">
