    ...
    <div data-sports-list-loading="true" className="w-full py-3">
      ...
      <div aria-hidden="true" className="space-y-2 px-1">
        {SPORTS_LIST_LOADING_LINE_WIDTHS.map((width, index) => (
          ...
          >
            <span
              className="h-3 w-16 shrink-0 animate-pulse rounded bg-surface-interactive/55"
              style={{ animationDelay: `${index * 80}ms` }}
            />
            <span
              className="h-4 max-w-2xl animate-pulse rounded bg-surface-interactive/70"
              style={{
                width,
            ...
            />
            <span
              className="ml-auto hidden h-3 w-20 shrink-0 animate-pulse rounded bg-surface-interactive/50 sm:block"
