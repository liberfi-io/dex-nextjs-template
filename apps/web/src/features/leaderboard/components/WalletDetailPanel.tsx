...
function WalletTabs({
  ...
  return (
    <section ref={sectionRef} className="flex flex-col">
      <div className="sticky top-0 z-10 -mx-px bg-surface-base/95 pb-2 pt-1 backdrop-blur">
        <div className="flex flex-col gap-2 border-b border-border-subtle/50 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            ...
            {tabs.map((tb) => (
              ...
              >
                ...
                {tb.count != null && tb.count > 0 && (
                  <span className="ml-1 text-text-muted">({tb.count})</span>
                )}
              </button>
            ))}
          </div>
          {tab !== "activity" && (
            <div className="max-sm:px-2 sm:contents">
              <input
                ...
                className="mb-1.5 w-full rounded-lg border border-border-subtle/60 bg-surface-raised/40 px-3 py-1.5 text-xs text-text-secondary outline-none placeholder:text-text-disabled focus:border-border-control sm:w-auto sm:min-w-[140px] sm:max-w-[220px] sm:flex-1"
              />
            </div>
          )}
        </div>
          ...
          >
            {virtualItems.map((vItem) => {
              ...
              return (
                ...
                >
                  {isLoaderRow ? (
                    <div className="flex items-center justify-center px-4 py-4 text-xs text-text-muted">
  ...
  if (src && !failed) {
    return (
      <img
        ...
        className={cn("flex-shrink-0 rounded-lg bg-surface-interactive object-cover", className)}
...
function PositionRow({
  ...
  return (
    <div className={cn("px-3 py-3", !last && "border-b border-border-subtle/40")}>
      <div className={cn("grid", TABLE_GRID, "items-center gap-1.5")}>
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            {display.subtitle && (
              <div className="line-clamp-1 text-[11px] uppercase tracking-wide text-text-disabled">
                {display.subtitle}
              </div>
            )}
          </div>
        ...
        <div className="text-right text-sm tabular-nums text-text-secondary">
        ...
        <LiveRelativeTime
          ...
          className="text-right text-xs tabular-nums text-text-muted"
        />
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {display.subtitle && (
          <div className="line-clamp-1 text-xs text-text-muted">{display.subtitle}</div>
        )}
        <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-text-muted">
          <ActivityTypeBadge type={activity.type} />
          {outcome && <span className="text-text-muted">{outcome}</span>}
...
function ActivityRowsSkeleton({ rows = 6 }: { rows?: number }) {
  return (
    <div className="overflow-hidden rounded-xl border border-border-subtle/40 bg-surface-raised/20">
      <div className="divide-y divide-border-subtle/40">
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="flex items-center gap-3 px-3 py-3">
            <div className="size-[34px] shrink-0 animate-pulse rounded-md bg-surface-interactive/50" />
            <div className="min-w-0 flex-1">
              <div className="mb-1.5 h-3.5 w-2/3 animate-pulse rounded bg-surface-interactive/50" />
              <div className="h-3 w-2/5 animate-pulse rounded bg-surface-interactive/50" />
            </div>
            <div className="h-4 w-16 shrink-0 animate-pulse rounded bg-surface-interactive/50" />
          </div>
        ))}
      </div>
    </div>
...
function EmptyBlock({ message, fill }: { message: string; fill?: boolean }) {
  return (
    <div
      className={cn(
        "flex items-center justify-center rounded-xl border border-border-subtle/40 bg-surface-raised/20",
