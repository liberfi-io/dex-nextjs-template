...
export function LeaderboardPage() {
  ...
  const updateParams = useCallback(
    (
      ...
      const url = qs ? `/predict/leaderboard?${qs}` : "/predict/leaderboard";
  ...
  const handleSelect = useCallback(
    (wallet: string) => {
      ...
      router.push(`/predict/leaderboard/${encodeURIComponent(wallet)}${qs}`, {
        scroll: false,
      });
    },
  ...
  const handlePrefetch = useCallback(
    (wallet: string) => {
      const qs = buildLeaderboardSearch({ interval, scope: activeScope });
      router.prefetch(`/predict/leaderboard/${encodeURIComponent(wallet)}${qs}`);
    },
    [activeScope, interval, router],
  );
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden">
      <div className="shrink-0 border-b border-border-subtle/60 bg-surface-base/95 backdrop-blur">
            ...
            <button
              ...
              className={cn(
                ...
                activeView === v
                  ...
                  : "text-text-muted hover:bg-surface-interactive/50 hover:text-text-secondary",
              )}
            >
      ...
      <div className="mx-auto flex w-full max-w-[1280px] min-h-0 flex-1 flex-col px-4 sm:px-6 lg:px-10 xl:px-12">
        <div className="flex shrink-0 flex-col gap-2 py-2 lg:flex-row lg:items-center lg:justify-between">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeScope === s
                      ...
                      : "border-border/80 text-text-muted hover:bg-primary/10 hover:text-primary",
                  )}
                >
                  {s === WORLDCUP_SCOPE && (
                    <svg
          ...
          {activeView === "smart-money" && (
            <div className="flex w-fit items-center gap-1 rounded-xl border border-border-subtle/60 bg-surface-base/40 p-1 backdrop-blur lg:shrink-0">
