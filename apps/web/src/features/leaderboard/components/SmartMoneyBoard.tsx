      ...
      <div className="custom-scrollbar flex min-h-0 flex-1 flex-col overflow-x-auto">
        <div className={cn(TABLE_MIN_W, "flex min-h-0 flex-1 flex-col")}>
          ...
          <div
            className={cn(
              ROW_GRID,
              "shrink-0 border-b border-border-subtle/50 px-3 py-2.5 text-[11px] font-medium uppercase tracking-wide text-text-muted",
            )}
          >
...
const BoardRow = memo(function BoardRow({
  ...
  return (
    <div
      ...
      className={cn(
        ROW_GRID,
        "group w-full cursor-pointer px-3 py-3 text-left outline-none transition-colors focus-visible:bg-surface-interactive/40",
    ...
    >
      ...
      {/* Balance */}
      <div className="text-right text-sm font-medium tabular-nums text-text-secondary">
        {formatUsd(entry.currentValue)}
      </div>

      {/* Volume / txs */}
      <div className="text-right">
        <div className="text-sm font-medium tabular-nums text-text-secondary">{formatUsd(vol)}</div>
        <div className="mt-0.5 text-xs text-text-muted">
...
function RankBadge({ position }: { position: number }) {
  ...
  return <span className="text-sm font-semibold tabular-nums text-text-muted">{position}</span>;
}

function BoardEmpty({ message }: { message: string }) {
  return (
    <div className="flex h-full min-h-0 w-full flex-col items-center justify-center gap-2 rounded-xl border border-border-subtle/40 bg-surface-raised/20 pb-4">
      <svg
        viewBox="0 0 24 24"
        width={32}
        height={32}
