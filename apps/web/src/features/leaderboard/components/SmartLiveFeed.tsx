...
export function SmartLiveFeed({ tag, scope }: { tag?: string | null; scope: LeaderboardScope }) {
        ...
        <button
          ...
          className={cn(
            ...
            filters.type === type
              ...
              : "border-border-subtle/80 text-text-muted hover:text-text-secondary",
          )}
        >
          {type === "all"
  ...
  return (
    <section className="flex min-h-0 min-w-0 flex-1 flex-col pb-4">
      <div className="flex shrink-0 flex-col gap-2 border-b border-border-subtle/60 py-3">
...
function PausedUpdatesIndicator() {
  return (
    <div className="flex h-8 w-7 shrink-0 items-center justify-center text-positive">
...
function RangeInput({
  ...
  return (
    <div
      className={cn(
        "shrink-0 rounded-lg border border-border-subtle/80 px-2 py-1",
        compact
    ...
    >
      ...
      <input
        ...
        placeholder="min"
        className={cn(
          "h-6 bg-transparent text-xs text-text-secondary outline-none placeholder:text-text-disabled",
...
function LiveFeedSelect({
  ...
  return (
    ...
    >
      ...
      {open && (
        <div
          className="z-50 overflow-hidden rounded-lg border border-border-subtle bg-surface-base shadow-2xl shadow-black/40"
          style={menuStyle}
        >
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="flex min-h-[360px] items-center justify-center text-sm font-medium text-text-muted">
      {label}
    </div>
  );
}
      ...
      className={cn(
        ...
        tone === "bullish"
          ...
          : tone === "bearish"
            ...
            : "bg-surface-interactive/70 text-text-secondary",
      )}
    >
  ...
  if (src && !failed) {
    return (
      <img
        ...
        className="shrink-0 rounded-lg bg-surface-interactive object-cover"
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-0.5 flex min-w-0 items-center gap-1.5">
            {display.subtitle && (
              <span className="min-w-0 truncate text-[11px] uppercase tracking-wide text-text-disabled">
                {display.subtitle}
              </span>
            )}
    ...
    >
      ...
      <div className="flex min-w-0 items-center gap-1.5">
        <button
          ...
          disabled={!row.wallet}
          className="min-w-0 cursor-pointer truncate text-left text-text-secondary transition-colors hover:text-text-primary hover:underline disabled:cursor-default disabled:text-text-disabled disabled:no-underline"
        >
          {traderLabel(row)}
        </button>
        {row.wallet && (
      ...
      <div className="text-right tabular-nums text-text-secondary">{formatPrice(row.price)}</div>
    </div>
  );
}
