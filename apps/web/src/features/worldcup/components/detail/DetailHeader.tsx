        ...
        <div className="flex min-w-0 items-start gap-3">
          {onBack && (
            <button
              ...
              className="flex size-10 shrink-0 cursor-pointer items-center justify-center rounded-lg text-text-secondary transition-colors hover:bg-surface-interactive/60 hover:text-white"
            >
              <svg
                width="18"
                height="18"
          ...
          {event.image_url ? (
            ...
            <div className="h-10 w-10 shrink-0 rounded-lg border border-border-subtle bg-surface-raised" />
          )}
          <div className="relative min-w-0">
            {onTogglePanel ? (
              ...
              >
                ...
                <span className="shrink-0 truncate text-base font-semibold text-brand-primary">
                  {selectedLabel}
                </span>
                <svg
                  width="16"
                  ...
                  strokeLinejoin="round"
                  className={cn(
                    "shrink-0 text-text-muted transition-transform group-hover:text-text-secondary",
    ...
    <div className="relative" ref={ref}>
      <button
        ...
        className={cn(
          ...
          open
            ? "border-border-control bg-surface-interactive text-text-primary"
            : "border-border-subtle bg-surface-raised/60 text-text-secondary hover:border-border-control hover:text-text-primary",
        )}
      >
        {icon}
        <span>{label}</span>
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-[min(92vw,380px)] overflow-hidden rounded-[12px] border border-border-subtle bg-surface-base shadow-2xl shadow-black/50">
          {children}
        </div>
      )}
    </div>
...
export function RulesContent({
  ...
  return (
    <div className="flex flex-col">
      <div className="px-4 pb-2 pt-3 text-sm font-semibold text-text-primary">
        {title}
      </div>
      <div className="max-h-[60vh] overflow-y-auto px-4 pb-4 text-[13px] leading-relaxed text-text-muted">
        {text ? (
