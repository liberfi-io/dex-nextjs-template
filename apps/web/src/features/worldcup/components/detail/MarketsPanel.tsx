...
export function MarketsPanel({
  ...
  return (
    <div
      className={cn(
        "flex min-h-0 flex-col rounded-[12px] border border-border-subtle bg-surface-raised/40",
        className,
      )}
    >
      {/* Category tabs */}
      {categoryTabs.length > 1 && (
        <div className="flex items-center gap-1 overflow-x-auto border-b border-border-subtle px-3 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
  ...
  return (
    <div className="flex flex-col gap-2">
            ...
            <button
              ...
              className={cn(
                ...
                selected
                  ...
                  : "border-border-control/60 text-text-secondary hover:border-border-control hover:text-text-primary",
              )}
            >
              ...
              <span className="text-sm font-bold leading-tight tabular-nums text-negative">
                {odds ?? "-"}
              </span>
            </button>
          );
      ...
      {lineOptions.length > 1 && (
        <div className="relative flex h-12 items-center overflow-hidden border-t border-border-subtle/70 bg-surface-base/60">
          <svg
            width="12"
            height="12"
            viewBox="0 0 12 12"
            aria-hidden
            className="pointer-events-none absolute left-1/2 top-[-3px] z-10 -translate-x-1/2 text-brand-primary"
          >
            <path
          ...
          <button
            ...
            className={cn(
              "absolute left-0 top-0 z-20 flex h-full w-8 cursor-pointer items-center justify-start bg-gradient-to-r from-surface-base/95 to-transparent pl-1 text-lg font-semibold text-text-muted transition-colors",
              activeLineIndex > 0
          ...
          <button
            ...
            onClick={() => selectAdjacentLine(1)}
            className={cn(
              "absolute right-0 top-0 z-20 flex h-full w-8 cursor-pointer items-center justify-end bg-gradient-to-l from-surface-base/95 to-transparent pr-1 text-lg font-semibold text-text-muted transition-colors",
...
function GroupRow({
  ...
  return (
    <div
      className="border-b border-border-subtle/60 pb-3 last:border-b-0 last:pb-0"
