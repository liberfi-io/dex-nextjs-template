...
export function MatchCenterTabs({
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-[12px] border border-border-subtle bg-surface-raised/40",
        className,
      )}
    >
      {!hideTabs && (
...
function CenterEmpty({ message }: { message: string }) {
  return (
    <div className="flex h-full min-h-[404px] flex-col items-center justify-center gap-2 text-sm text-text-muted">
      <EmptyIcon width={28} height={28} className="text-text-disabled" />
      <span>{message}</span>
    </div>
  );
}
