...
export function WorldCupDetailPage({
  ...
  if (!event) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-3 text-sm text-text-muted">
        {t("extend.worldcup.detail.notFound")}
        <button
          type="button"
          onClick={() => router.back()}
          className="rounded-lg border border-border-control px-3 py-1.5 text-xs text-text-secondary hover:text-text-primary cursor-pointer"
        >
    ...
    return (
      <div className="flex w-full flex-col gap-4">
        ...
        <div ref={mobileTabsSectionRef} className="flex flex-col gap-3">
          <div className="flex items-center gap-1 overflow-x-auto rounded-[10px] border border-border-subtle bg-surface-raised/60 p-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            >
              <MarketsPanel
                ...
                renderInlineOrderbook={(slug, inlineOutcome) => {
                  ...
                  return (
                    <div
                      className="flex min-h-[320px] flex-col rounded-[12px] border border-border-subtle bg-surface-raised/40 p-3"
        ...
        {selectedMarket && (
          <TradeModal
            ...
            hideHeader
            contentClassName="!max-h-[min(620px,calc(100dvh-48px))] overflow-hidden !rounded-t-[18px] !border !border-border-subtle !bg-surface-interactive"
    ...
    <div className="mx-auto flex w-full max-w-[1760px] flex-col gap-4 lg:flex-row lg:items-start">
      ...
      {selectedMarket && (
        <aside className="flex w-full shrink-0 flex-col gap-4 lg:sticky lg:top-2 lg:w-[340px] xl:w-[360px]">
          <div className="rounded-[12px] border border-border-subtle bg-surface-raised/40 p-3">
            <TradePanel
              event={displayEvent}
              market={tradeMarket ?? selectedMarket}
              outcome={outcome}
          ...
          <div className="flex min-h-[360px] flex-col rounded-[12px] border border-border-subtle bg-surface-raised/40 p-3">
            <EventMarketDetailWidget
...
function MarketSwitcherFrame({
  ...
  return (
    <div
      className={cn(
        "flex min-h-0 flex-col overflow-hidden rounded-[12px] border border-border-subtle bg-surface-base",
        className,
      )}
    >
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {(onAction || onClose) && (
            <button
              ...
              title={buttonLabel}
              className="flex h-6 w-6 cursor-pointer items-center justify-center rounded-md text-text-muted transition-colors hover:bg-surface-interactive hover:text-text-secondary"
            >
