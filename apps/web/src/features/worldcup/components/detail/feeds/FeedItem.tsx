...
export function FeedItem({ feed }: { feed: WcFeed }) {
  ...
  return (
    <article className="flex flex-col gap-2 rounded-[10px] border border-border-subtle bg-surface-raised/40 p-3">
      {isRepost && (
      ...
      {text && isLong && (
        <button
          ...
          className="self-start text-xs font-medium text-brand-primary hover:underline cursor-pointer"
        >
          {expanded
