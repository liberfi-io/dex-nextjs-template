...
function CountdownCell({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <span className="rounded-md bg-surface-interactive px-2 py-1 text-sm font-bold tabular-nums text-text-primary">
        {value}
      </span>
...
export function MatchBanner({ match }: { match: WcMatch }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-[12px] border border-border-subtle bg-surface-raised/40 px-4 py-3">
      ...
      <div className="flex min-w-0 flex-1 flex-col items-center gap-1.5 text-center">
        ...
        {!showLiveState && (
          <div className="text-xs font-bold tabular-nums text-brand-primary">
            {homeProb}%
          </div>
        )}
      </div>
