...
export function LiveStreamPanel({
  ...
  return (
    <div className={cn("flex min-h-0 flex-col gap-2", className)}>
      ...
      <div
        ref={playerRef}
        className="aspect-video w-full overflow-hidden rounded-xl border border-border-subtle bg-surface-base"
      >
        {!hasPlayableSource || !current || iframeState === "failed" ? (
          ...
          <iframe
            ...
            className={cn("h-full w-full border-0 bg-surface-base", iframeClassName)}
          ...
          <div className="flex h-full w-full items-center justify-center">
            <span className="h-7 w-7 animate-spin rounded-full border-2 border-border-control border-t-brand-primary" />
          </div>
        )}
      </div>
    </div>
