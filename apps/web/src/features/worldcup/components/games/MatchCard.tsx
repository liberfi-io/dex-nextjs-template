  ...
  return (
    <div className="space-y-3 p-1">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="h-12 animate-pulse rounded-[10px] bg-surface-interactive/50" />
      ))}
    </div>
  );
}
...
function PanelEmpty({ message }: { message: string }) {
  return (
    <div className="flex min-h-32 items-center justify-center rounded-[10px] border border-border-subtle bg-surface-base/30 text-sm text-text-muted">
      {message}
    </div>
  );
}
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2">
      {items.map((item) => (
        <div
          key={item.key}
          className="flex min-h-[76px] items-center gap-3 rounded-[12px] border border-border-subtle bg-surface-base/30 px-3"
        >
  ...
  return (
    <div className="space-y-1.5">
      <div className="grid grid-cols-[56px_minmax(0,1fr)_56px] items-center gap-2 text-xs">
        <span className="font-semibold tabular-nums text-text-secondary">{fmt(left)}</span>
        <span className="text-center text-text-muted">{label}</span>
        <span className="text-right font-semibold tabular-nums text-text-secondary">{fmt(right)}</span>
      </div>
      <div className="grid h-1.5 grid-cols-2 overflow-hidden rounded-full bg-surface-interactive">
        <div className="flex justify-end bg-surface-interactive">
          <div className="h-full rounded-l-full bg-primary" style={{ width: leftWidth }} />
        </div>
        <div className="bg-surface-interactive">
          <div className="h-full rounded-r-full bg-surface-emphasis" style={{ width: rightWidth }} />
        </div>
      </div>
    </div>
  );
...
function TechnicalStatsPanel({
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 text-sm font-semibold text-text-primary">
        <div className="flex min-w-0 items-center gap-2">
...
function resultTone(result?: string): string {
  ...
  return "bg-surface-interactive text-text-secondary";
}
...
function TeamFormBlock({
  ...
  return (
    <div className="space-y-3 rounded-[12px] border border-border-subtle bg-surface-base/30 p-3">
...
function HeadToHeadRow({
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 rounded-[8px] bg-surface-raised/50 px-3 py-2 text-xs">
      <span className="truncate text-text-secondary">{teamNameByCode(t, match, item.homeCode)}</span>
  ...
  return (
    <div className="flex min-w-0 items-center gap-2 text-sm">
      {player.number !== undefined ? (
        <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-surface-interactive px-1 text-xs font-semibold text-text-muted">
          {player.number}
        </span>
      ) : (
      ...
      {player.position ? (
        <span className="shrink-0 text-xs text-text-muted">{player.position}</span>
      ) : null}
    </div>
  );
}
...
function FullLineupPanel({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {squads.map((squad) => (
        <div key={squad.teamCode} className="space-y-3 sm:border-r sm:border-border-subtle sm:pr-4 sm:last:border-r-0 sm:last:pr-0">
          <SquadHeader squad={squad} match={match} t={t} />
          <div className="space-y-2">
...
function MatchCardImpl({
  ...
  const renderLiveButton = (opts: {
    ...
    <button
      ...
      className={cn(
        ...
        isLive
          ? "border-negative/50 bg-negative/15 text-negative hover:bg-negative/20"
          : match.status === "final"
          ...
          : opts.highlighted
            ...
            : "border-border-control/60 bg-surface-interactive/50 text-text-secondary hover:bg-surface-interactive",
        opts.className,
      )}
    >
      {isLive ? (
  ...
  const viewPill = (
    <span className="hidden shrink-0 items-center gap-1 rounded-full border border-border-control/60 bg-surface-interactive/50 px-2.5 py-1 text-[11px] font-medium text-text-secondary transition-colors group-hover:bg-surface-interactive md:flex">
  ...
  return (
    <div
      ...
      className={cn(
        "group cursor-pointer overflow-hidden rounded-[14px] border bg-surface-raised/40 transition-colors [content-visibility:auto] [contain-intrinsic-size:auto_140px] hover:border-border-control/80",
        highlighted
          ? "border-brand-primary/70 shadow-[0_0_0_1px_rgba(199,255,46,0.35)]"
        ...
        {widgetOpen && (
          ...
          >
            <div className="px-3 pb-3 sm:px-4">
              <div className="rounded-[12px] border border-border-subtle bg-surface-raised/40">
                <div className="flex items-center gap-1 overflow-x-auto border-b border-border-subtle px-2 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
