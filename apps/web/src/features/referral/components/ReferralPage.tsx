    ...
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <button
          type="button"
          onClick={signIn}
          className="rounded-[10px] border border-brand-primary/25 bg-brand-primary/10 px-4 py-2 text-sm font-semibold text-brand-primary transition-colors hover:bg-brand-primary/20"
        >
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6 pb-24 lg:pb-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4 lg:order-1">
          ...
          {/* Tabs */}
          <div className="flex items-center gap-5 border-b border-border-subtle px-1">
            <TabButton
              active={tab === "invited"}
  ...
  return (
    <div className="flex flex-1 flex-col gap-2 px-4 py-4" style={CARD_STYLE}>
      ...
      <span className="text-[11px] text-text-muted">
        {lockedLabel}: <span className="tabular-nums text-text-muted">{lockedValue}</span>
      </span>
      <button
        type="button"
        ...
        className={cn(
          ...
          canClaim && !claiming
            ? "cursor-pointer bg-brand-primary/10 text-brand-primary hover:bg-brand-primary/20"
            : "cursor-not-allowed bg-surface-interactive/60 text-text-muted",
        )}
      >
        {claiming ? claimingLabel : claimLabel}
      </button>
      {!canClaim && <span className="text-[10px] text-text-disabled">{minHint}</span>}
    </div>
  );
}
...
function InviteesList({
  ...
  return (
    <div className="min-w-[560px] lg:min-w-0">
      <div className="flex items-center justify-between border-b border-border-subtle px-4 py-2.5 text-[11px] uppercase tracking-wide text-text-muted">
      ...
      {rows.map((r) => (
        <div
          ...
          className="flex items-center justify-between px-4 py-3 text-sm hover:bg-surface-interactive/60"
        >
          <span className="w-[180px] shrink-0 font-medium text-text-secondary">
            {truncateAddress(r.invitee_user_address)}
          </span>
          <span className="flex-1 text-right tabular-nums text-text-secondary">
          ...
          <span className="w-[120px] shrink-0 text-right text-xs text-text-muted">
...
function EmptyRow({ text }: { text: string }) {
  return <div className="px-4 py-10 text-center text-sm text-text-muted">{text}</div>;
}
  ...
  return (
    <div className="flex flex-col gap-4 px-5 py-5" style={CARD_STYLE}>
      ...
      <div>
        ...
        <div className="mt-1 rounded-[10px] bg-surface-interactive/60 px-3 py-2.5 text-center text-lg font-bold tracking-widest text-text-primary">
          {inviteCode || "—"}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mt-1 flex items-center gap-2 rounded-[10px] bg-surface-interactive/60 px-3 py-2">
          <span className="flex-1 truncate text-xs text-text-muted">{inviteLink || "—"}</span>
          <button
            type="button"
            onClick={handleCopy}
            disabled={!inviteLink}
            className="shrink-0 rounded-md px-2 py-1 text-xs font-medium text-text-secondary transition-colors hover:bg-surface-strong disabled:opacity-50"
          >
      ...
      <button
        ...
        onClick={handleShare}
        disabled={!inviteLink}
        className="rounded-[10px] bg-brand-primary/10 px-4 py-2.5 text-sm font-semibold text-brand-primary transition-colors hover:bg-brand-primary/20 disabled:cursor-not-allowed disabled:opacity-50"
      >
