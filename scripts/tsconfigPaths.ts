...
async function main() {
  ...
  for (const pkg of packages) {
    // Get dependencies that belong to the "@liberfi.io" or "@liberfi/" scope
    const deps = getLiberfiDependencies(pkg.packageJson);
    ...
    for (const dep of deps) {
      ...
      if (relativeDir) {
        // Exact match: import from "@liberfi/xxx"
        paths[dep] = [relativeDir];
        // Wildcard match: import from "@liberfi/xxx/libs/format" etc.
        paths[`${dep}/*`] = [`${relativeDir}/*`];
      }
    }
...
/**
 * Extracts dependencies from package.json that belong to the "@liberfi.io" or "@liberfi/" scope.
 * This includes both external npm packages (@liberfi.io/) and workspace packages (@liberfi/).
 */
function getLiberfiDependencies(packageJson: Package["packageJson"]) {
  ...
  return Object.keys(allDependencies).filter(
    (name) => name.startsWith("@liberfi.io/") || name.startsWith("@liberfi/"),
  );
}
